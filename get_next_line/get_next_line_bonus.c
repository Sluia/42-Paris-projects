...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/22 09:18:32 by qduarte           #+#    #+#             */
/*   Updated: 2020/11/22 09:19:34 by qduarte          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(int fd, char **line)
{
	...
	*line = NULL;
	...
	if (buf_len > 0)
	{
		return_val = chk_buf_read(buffer[fd], &buf_len, line);
		if (return_val == 1 || return_val == -1)
			return (return_val);
	}
	buf_len = (int)read(fd, buffer[fd], BUFFER_SIZE);
	while (buf_len > 0)
	{
		return_val = chk_buf_read(buffer[fd], &buf_len, line);
		if (return_val == 1 || return_val == -1)
